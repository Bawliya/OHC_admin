...
import WithdrawlComponent from "../../../components/WithdrawlTableComponent";
import { getWithdrawlFunction } from "../../../services/WithdrawlRequest/WithdrawlRequest";

const WithdrawlTablePage: React.FC = () => {
    ...
    const [type, setType] = useState<any>("Restaurant")

    useEffect(() => {
        getBanners(currentPage, 10, status, type);
    }, [currentPage, status, type]); // Refetch data when the ccurrentPage: numberurrentPage changes

    const getBanners = async (currentPage: any, limit: any, status: any, type: any) => {
        const resp: any = await getWithdrawlFunction(currentPage, limit, status, type); // Pass currentPage to API call
...
export default WithdrawlTablePage;
