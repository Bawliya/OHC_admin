...
const ZoneEditModal: React.FC<props> = ({
  ...
  const [paymentMethods, setPaymentMethods] = useState({
    cash_on_delivery: currentData?.cash_on_delivery === "true" ? true : false,
    digital_payment: currentData?.digital_payment === "true" ? true : false,
    offline_payment: currentData?.offline_payment === "true" ? true : false,
  });
  const [increaseDeliveryCharge, setincreaseDeliveryCharge] = useState<any>(
    currentData?.increaseDeliveryCharge
  );
  ...
  const handleSubmitForm = async (event: any) => {
    ...
    if (res?.data?.status) {
      ...
      const updatedData = table?.map((item: any) =>
